...
- **Resolved.** The Form 3500 field list is pinned in
  `src/lib/form-3500-fields.ts`, built directly from the current,
  authoritative FDA PDF. That work also corrected the section-level
  structure itself, which the original draft of this document got wrong
  (see Data shape above) — not just under-specified below the section
