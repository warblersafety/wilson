...
- **Writes follow the clinician's own state.** `unasked` fields the
  ...
  **One-hot groups are the exception, and are handled at fact
  ...
  authoritative).** A write to an `exclusive` group is one atomic write
  of the whole fact, so three things follow. It supersedes prior
  ...
  checked. And naming the fact's value in the visible reply ("Also
